...
impl SpawnManager {
    ...
    pub fn run_spawning(&mut self, room: &Room, cache: &mut CachedRoom, memory: &mut ScreepsMemory) {
        ...
        self.spawn_queue.sort_by(|a, b| b.priority.partial_cmp(&a.priority).unwrap());

        for spawn in available_spawns {
            let request = self.spawn_queue.pop().unwrap();
            if self.can_spawn(room, &request) {
                let options = request.spawn_options.unwrap_or(SpawnOptions::default());
                ...
                spawn.spawn_creep_with_options(&request.body, &creep_name, &options);
            }
        }
    }
