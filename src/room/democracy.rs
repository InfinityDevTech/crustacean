...
pub fn start_government(room: Room, memory: &mut ScreepsMemory, cache: &mut RoomCache) {
    ...
    if room.my() {
...

        {
            ...
            let pre_order_creation = game::cpu::get_used();
            ...
            cached_room.stats.cpu_hauling_orders = game::cpu::get_used() - pre_order_creation;

            let pre_link_cpu = game::cpu::get_used();
            links::balance_links(&room, cached_room);
            cached_room.stats.cpu_links = game::cpu::get_used() - pre_link_cpu;
            ...
            let pre_tower_cpu = game::cpu::get_used();
            tower::run_towers(cached_room);
            cached_room.stats.cpu_towers = game::cpu::get_used() - pre_tower_cpu;
        }

        // Makes hauling requests for the rooms remotes :)
        let pre_remote_cpu = game::cpu::get_used();
        resources::haul_remotes(&room, memory, cache);
        cache.rooms.get_mut(&room.name()).unwrap().stats.cpu_hauling_orders += game::cpu::get_used() - pre_remote_cpu;

        let pre_creep_cpu = game::cpu::get_used();
        organizer::run_creeps(&room, memory, cache);
        cache.rooms.get_mut(&room.name()).unwrap().stats.cpu_creeps = game::cpu::get_used() - pre_creep_cpu;
...

        {
            ...
            let pre_remote_scan = game::cpu::get_used();
            ...
            cache.rooms.get_mut(&room.name()).unwrap().stats.cpu_remotes = game::cpu::get_used() - pre_remote_scan;
        }
    } else {
